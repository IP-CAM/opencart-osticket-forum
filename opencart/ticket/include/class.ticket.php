...
require_once(INCLUDE_DIR.'plugins/oc_integration/class.oc_customer.php');//for hunt_easter
...
class Ticket {
    ...
    function checkUserAccess($user) {
        //checking this first for hunt_easter
    ...
    //for hunt_easter
    if (!isset($this->owner))
        $this->owner = new Oc_customer($this->ht['user_id']);

        return $this->owner;
    }

    function getEmail(){
        //START CHANGE for hunt_easter
        if ($o = $this->getOwner()) {
            return $o->getEmail();
        } elseif ($customer = new Oc_customer($this->getOwnerId())) { //for hunt_easter
            return $customer->getEmail();
        }
        //END CHANGE FOR HUNT_EASTER
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    function create($vars, &$errors, $origin, $autorespond=true, $alertstaff=true) {
        global $ost, $cfg, $thisclient, $_FILES, $customer; //for hunt_easter
        ...
        if (!$errors) {
            ...
            if (!$user && !$customer) { //checking $customer for hunt_easter
                $user_form = UserForm::getUserForm()->getForm($vars);
        ...
        if ( $customer && $customer->getId() ) { //for hunt_easter
            $sql='INSERT INTO '.TICKET_TABLE.' SET created=NOW() '
            .' ,lastmessage= NOW()'
            .' ,user_id='.db_input($customer->getId())
